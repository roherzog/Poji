...
Dialog.addString("Channel 3:", "MyosinIIA"); //Change "MyosinIIA" for preferred name for channel 3
...
Chan1_temp=File.getParent(Chan1) + fs + "Analysis_temp_" + Channame1 + fs;
File.makeDirectory(Chan1_temp);
if (Channumber>1) {
	Chan2=getDirectory("Select source folder for " + Channame2 + " images");
	list2=getFileList(Chan2);
	Chan2_temp=File.getParent(Chan2) + fs + "Analysis_temp_" + Channame2 + fs;
	...
	if (Channumber>2) {
		...
		Chan3_temp=File.getParent(Chan3) + fs + "Analysis_temp_" + Channame3 + fs;
		...
		if (Channumber>3) {
			...
			Chan4_temp=File.getParent(Chan4) + fs + "Analysis_temp_" + Channame4 + fs;
...
Dialog.addCheckbox("also save profiles sorted by channels?", true); 
...
for (aaa=0; aaa<list1.length; aaa++) {
	if (!File.isDirectory(Chan1+list1[aaa])) {
		...
		for (abc=1; abc<=nRois; abc++) {
			...
			close();
			if (Channumber>1) {
				if (!File.isDirectory(Chan2+list2[aaa])) {
				...
				if (Channumber>2) {
					if (!File.isDirectory(Chan3+list3[aaa])) {
					...
					if (Channumber>3) {
						if (!File.isDirectory(Chan4+list4[aaa])) {
...
for (aaa=0; aaa<list1_temp.length; aaa++) {
	...
	while (preview==1) {
		if (same_detec==0 || aaa==0) {
			...
			if (u_prev==0) {
				Dialog.addMessage("detected podosomes = ..." );
			} else {
...
for (bbb=0; bbb<list1_temp.length; bbb++) {
	if (!File.isDirectory(list1_temp[bbb])) {
		...
		selectWindow("Results");
		run("Close");
		File.makeDirectory(Output + name1 + fs + "Images");
		selectImage(Channame1);
		if (norm_cell==0) {
			saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Podosomestack " + Channame1 + ".tiff");
		} else {
			saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Podosomestack " + Channame1 + " normalised.tiff");
		}
		close();
		...
		if (data_save=="all") {
			if (norm_cell==0) {
				saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Podosome covered area " + Channame1 + ".tiff");
			} else {
				saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Podosome covered area " + Channame1 + " normalised.tiff");
			}
		}
		close();
		...
		//Profile analysis
		if (profile==1) {
			File.makeDirectory(Output + name1 + fs + "Profiles");
			File.makeDirectory(Output + name1 + fs + "Profiles" + fs + "Images");
			if (data_save=="all") {
				File.makeDirectory(Output + name1 + fs + "Profiles" + fs + "Single rotations");
			}
			if (bit==8) {
			...
			if (single_profiles==1) {
				File.makeDirectory(Output + name1 + fs + "Profiles" + fs + "Individual profiles");
				if (data_save=="all") {
					File.makeDirectory(Output + name1 + fs + "Profiles" + fs + "Individual profiles" + fs + "Single rotations");
				}
			...
			if (data_save=="all") {
				...
				if (norm_cell==0) {
					saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Profilestack " + Channame1 + ".tiff");
				} else {
					saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Profilestack " + Channame1 + " normalised.tiff");
				}
				close();
			}
			for (f=1; f<=u; f++) {
			...
			if (data_save=="all") {
				if (norm_cell==0) {
					saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Profilestack " + Channame1 + " (normed).tiff");
				} else {
					saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Profilestack " + Channame1 + " (normed) and normalised.tiff");
				}
			}
			close();
		...
		run("Select None");
		if (norm_cell==0) {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Cell " + Channame1 + ".tiff");
		} else {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Cell " + Channame1 + " normalised.tiff");
		}
		close();
		selectImage(name1);
		run("Select None"); 
		if (norm_cell==0) {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Source image " + Channame1 + ".tiff");
		} else {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Source image " + Channame1 + " normalised.tiff");
		}
		close();
		roiManager("Reset"); 
		...
		setResult(Cell_name, 8, "single podosome intensities");
		for (res=0; res<u; res++) {
	...
	if (profile==1) {
		...
		selectWindow("Results"); 
		saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Profiles (original)" + data_ending);
		run("Close");
 		IJ.renameResults("Prof_stack normed", "Results");
		selectWindow("Results"); 
		saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Profiles (stack normed)" + data_ending);
		run("Close");
		IJ.renameResults("Prof_z-projection and stack normed", "Results");
		selectWindow("Results"); 
		saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Profiles (z-projection and stack normed)" + data_ending);
		run("Close");
		IJ.renameResults("Prof_stDev_original", "Results");
		selectWindow("Results");
		if (data_save=="all") {
			saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Single rotations" + fs + "Mean with stDev of single rotations (original)" + data_ending);
		}
		run("Close");
		IJ.renameResults("Prof_stDev_stack normed", "Results");
		selectWindow("Results");
		if (data_save=="all") {
			saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Single rotations" + fs + "Mean with stDev of single rotations (stack normed)" + data_ending);
		}
		run("Close");
		IJ.renameResults("Prof_stDev_z-projection and stack normed", "Results");
		selectWindow("Results");
		if (data_save=="all") {
			saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Single rotations" + fs + "Mean with stDev of single rotations (z-projection and stack normed)" + data_ending);
		}
		run("Close");
		if (single_profiles==1) {
			IJ.renameResults("Single profs_original", "Results");
			selectWindow("Results"); 
			saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Individual profiles" + fs + "Mean with stDev of individual profiles (original)" + data_ending);
			run("Close");
			IJ.renameResults("Single profs_stack normed", "Results");
			selectWindow("Results"); 
			saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Individual profiles" + fs + "Mean with stDev of individual profiles (stack normed)" + data_ending);
			run("Close");
		}
		run("Set Measurements...", "integrated redirect=None decimal=0");
	}
...
if (p_sorted==1 && profile==1) {
	run("Set Measurements...", "integrated redirect=None decimal=" + decimal);
	result_list=getFileList(Output);
	for (res=0; res<result_list.length; res++) {
		if (File.isDirectory(Output + result_list[res])) {
			run("Results... ", "open=[" + Output + result_list[res] + "Profiles" + fs + "Profiles (original)" + data_ending + "]");
			nres=nResults;
	...
	for (save_res=0; save_res<headings.length; save_res++) {
		...
		saveAs("Results", Output + "Combined results" + fs + "Profiles (" + headings[save_res] + ")" + data_ending);
		run("Close"); 
	}
}
...
if (!File.exists(Output + "Combined results")) {
	File.makeDirectory(Output + "Combined results");
}
selectWindow(Channame1);
saveAs("Results", Output + "Combined results" + fs + "Intensities (" + Channame1 + ")" + data_ending);
run("Close");
File.delete(Chan1_temp);
if (Channumber>1) {
	selectWindow(Channame2);
	saveAs("Results", Output + "Combined results" + fs + "Intensities (" + Channame2 + ")" + data_ending);
	run("Close");
	File.delete(Chan2_temp);
	if (Channumber>2) {
		selectWindow(Channame3);
		saveAs("Results", Output + "Combined results" + fs + "Intensities (" + Channame3 + ")" + data_ending);
		run("Close");
		File.delete(Chan3_temp);
		if (Channumber>3) {
			selectWindow(Channame4);
			saveAs("Results", Output + "Combined results" + fs + "Intensities (" + Channame4 + ")" +  data_ending);
			run("Close");
			File.delete(Chan4_temp);
		}
	}
...
function Analysis(source1, source2, protein) {	
	if (!File.isDirectory(source2+source1[bbb])) {
		...
		selectImage(protein);
		if (norm_cell==0) {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Podosomestack " + protein + ".tiff");
		} else {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Podosomestack " + protein + " normalised.tiff");
		}
		close();
		roiManager("Reset");
		...
		if (data_save=="all") {
			if (norm_cell==0) {
				saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Podosome covered area " + protein + ".tiff");
			} else {
				saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Podosome covered area " + protein + " normalised.tiff");
			}
		}
		close();
		selectWindow("Results");
		...
		if (profile==1) { 
			...
			if (data_save=="all") {
				run("Duplicate...", "duplicate");
				if (norm_cell==0) {
					saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Profilestack " + protein + ".tiff");
				} else {
					saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Profilestack " + protein + " normalised.tiff");
				}
				close();
			}
			for (j=1; j<=u; j++) {
			...
			if (data_save=="all") {
				if (norm_cell==0) {
					saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Profilestack " + protein + " (normed).tiff");
				} else {
					saveAs("Tiff",  Output + name1 + fs + "Images" + fs + "Profilestack " + protein + " (normed) and normalised.tiff");
				}
			}
			close();
			roiManager("Reset");
		...
		run("Select None");
		if (norm_cell==0) {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Cell " + protein + ".tiff");
		} else {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Cell " + protein + " normalised.tiff");
		}
		close();
		selectImage(name2);
		run("Select None"); 
		if (norm_cell==0) {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Source image " + protein + ".tiff");
		} else {
			saveAs("Tiff", Output + name1 + fs + "Images" + fs + "Source image " + protein + " normalised.tiff");
		}
		close(); 
		roiManager("Reset"); 
...
function do_single_profiles(channel_name, norm_status) {
	...
	for (a_single=1; a_single<=n_single; a_single++) {
		...
		selectWindow("Results"); 
		if (data_save=="all") {
			saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Individual profiles" + fs + "Single rotations" + fs + "Podosome " + a_single + " " + channel_name + " (" + norm_status + ")" + data_ending);
		} 			
	}
	run("Close");
	if (data_save=="all") {
		...
		updateResults();
		selectWindow("Results");
		saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Individual profiles" + fs + "Single rotations" + fs + "Mean with stDev of single rotations " + channel_name + " (" + norm_status + ")" + data_ending);
		run("Close");
	}
	for (sa=0; sa<n_single; sa++) {
		for (sb=0; sb<width_single; sb++) {
	...
	selectWindow("Results");
	saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Individual profiles" + fs + "Individual profiles " + channel_name + " (" + norm_status + ")" + data_ending);
	run("Close");
...
function do_profile(iname, def, dont_select) {
	...
	selectWindow("Results");
	if (data_save=="all") {
		saveAs("Results", Output + name1 + fs + "Profiles" + fs + "Single rotations" + fs + "Single rotations " + iname + " (" + def + ")" + data_ending);
	}
	run("Close");
	run("Set Measurements...", "integrated redirect=None decimal=0");
	if (iname!=Channame1) {
	...
	selectImage(podavg);
	saveAs("Tiff", Output + name1 + fs + "Profiles" + fs + "Images" + fs + "Z-projection " + iname + " (" + def + ").tiff");
}