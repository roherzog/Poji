...
for (a=0; a<n; a++) {
	...
	for (stack=0; stack<slices; stack++) {
		for (colour=1; colour<=channels; colour++) {
			...
			run("Paste");
			run("Select None"); 
			if (slices >10 && stack<10) {
				if (colour==1) {
					saveAs("Tiff", Output + Chan_1 + fs + name + " Z=0" + stack + " C=" + colour + ".tiff");
				} else if (colour==2) {
					saveAs("Tiff", Output + Chan_2 + fs + name + " Z=0" + stack + " C=" + colour + ".tiff");
				} else if (colour==3) {
					saveAs("Tiff", Output + Chan_3 + fs + name + " Z=0" + stack + " C=" + colour + ".tiff");
				} else if (colour==4) {
					saveAs("Tiff", Output + Chan_4 + fs + name + " Z=0" + stack + " C=" + colour + ".tiff");
				}
			} else {
				if (colour==1) {
					saveAs("Tiff", Output + Chan_1 + fs + name + " Z=" + stack + " C=" + colour + ".tiff");
				} else if (colour==2) {
					saveAs("Tiff", Output + Chan_2 + fs + name + " Z=" + stack + " C=" + colour + ".tiff");
				} else if (colour==3) {
					saveAs("Tiff", Output + Chan_3 + fs + name + " Z=" + stack + " C=" + colour + ".tiff");
				} else if (colour==4) {
					saveAs("Tiff", Output + Chan_4 + fs + name + " Z=" + stack + " C=" + colour + ".tiff");
				}
			}
			close();
		}
